...
class BaseRetailerService {
  ...
  getConfigs() {
    ...
    let configs = {
      ...
      CONNECTOR_TYPE: process.env.CONNECTOR_TYPE,
...
baseRetailerService.parse(parseFun)
  ...
  generateTask({ url, priority, metadata, suitableProducers, globalId } = {}) {
    ...
    if (!metadata || !metadata.script) {
      suitableProducers.push("SERVICE");
    }
 ...
 * Configurations Schema
   ...
   @property {string} LOG_FILES_PATH=public/log                 - Path to your log folder. You can view it by open `http://localhost:{PORT}/log/{COMBINED_LOG_FILE_NAME}` and `http://localhost:{PORT}/log/{ERROR_LOG_FILE_NAME}`. Default `node_modules/@bitskyai/retailer-sdk/lib/public/log`
   ...
   @property {string} DATA_PATH=public/data.json                - Path to `data.json`, this is the default way to store data in your disk. Default `node_modules/@bitskyai/retailer-sdk/lib/public/data.json`
 */
