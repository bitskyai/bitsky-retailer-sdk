 ...
 * @returns {Promise}
 */
function sendToMunewEngine(tasks) {
  let reqConfig = {
    //**********************************
    // YOU MUST DO 1
    //**********************************
...
function generateIntelligence(url, priority, metadata) {
  return {
    soi: {
      ...
      globalId: process.env.GLOBAL_ID||"c29pOjoxNTgxNzM5MzUzNTc1OjpjNTFiODZlNS1iMTQ5LTRhYmItYTA5YS0xZmY5NDRiMWVmYzY="
    },
...
router.get("/trigger", function(req, res, next) {
  ...
  let number = process.env.INIT_TASKS_NUMBER || 1;
...
router.post("/", function(req, res, next) {
  try {
    ...
    // when you received tasks, you should return 200. 
    res.status(200).end();
  }
});
