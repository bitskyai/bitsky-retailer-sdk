  ...
  function initRouter({ baseRetailerService, parse, trigger }) {
    ...
    router.get("/trigger", async function (req, res, next) {
      try {
        ...
        baseRetailerService.logger.error(`GET /trigger fail. Error: ${err.message}`, {error: err});
        res.status(500).json({
    ...
    router.get("/queue", async function (req, res, next) {
      try {
        ...
        baseRetailerService.logger.error(`GET /queue fail. Error: ${err.message}`, {error: err});
        res.status(500).json({
          message: _.get(err, "message"),
        });
      }
    ...
    router.post("/", async function (req, res, next) {
      try {
        ...
        baseRetailerService.logger.error(`POST / fail. Error: ${err.message}`, {error: err});
        res.status(500).json({
          message: _.get(err, "message"),
        });
      }
